...
class Demo extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      useBodyScroll: true,
      getClinicActivityList:getCaseList,
      data:[]
    };
  }
  ...
  _renderRow(row, sectionId, rowId) {
    return (
      <div key={row.id}
        ...
        >
          <div style={{ textAlign:'center',paddingTop:'20px',paddingBottom:'10px', color: '#000', fontSize: '18px'}}>
            {row.caseName}
          </div>
          <div style={{ display: '-webkit-box',  padding: '10px' }}    onClick={this.getClick.bind(this,row.id)}>
            <img style={{ width:'100%' ,border:"1px solid #dbdbdb"}} src={row.casePicture} alt="" />
          </div>
        </div>
    )
  }
